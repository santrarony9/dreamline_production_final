...
export async function generateMetadata({ params }) {
    ...
    try {
        ...
        if (slug.match(/^[0-9a-fA-F]{24}$/)) {
            post = await Journal.findById(slug).lean();
        } else {
            post = await Journal.findOne({ id: slug }).lean();
        }
        if (!post) return {
    ...
    return (
        <main className="bg-black pt-32 min-h-screen">
            ...
            <section className="container mx-auto px-6 mb-16">
                <Link href="/blogs" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 hover:text-[#c5a059] transition-colors mb-10 group interactive">
                    ...
                    <div className="mt-20 pt-12 border-t border-white/5 flex justify-between items-center">
                        <div className="flex gap-6 items-center">
                            ...
                            <div className="flex gap-4">
                                <a 
                                    href={`https://www.facebook.com/sharer/sharer.php?u=https://dreamlineproduction.com/blogs/`} 
                                    target="_blank" 
                                ...
                                <a 
                                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=https://dreamlineproduction.com/blogs/`} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="text-gray-500 hover:text-white transition-colors"
                                >
                                ...
                                <a 
                                    href={`https://www.linkedin.com/sharing/share-offsite/?url=https://dreamlineproduction.com/blogs/`} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="text-gray-500 hover:text-white transition-colors"
                                >
